			...
			local function hasAvailableAmberModel()
				...
				for _, AmberSpawn in ipairs(AmberFolder:GetChildren()) do
					...
					if model and model:FindFirstChild("Main") and model.Main:FindFirstChild("ProximityPrompt") then
						if model.Main.ProximityPrompt.Enabled and not isPlayerNear(model.Main.Position, 100) then
							return true
						end
					end
				end
			...
			local function hasAvailableGoat()
				...
				for _, goat in ipairs(GoatsFolder:GetChildren()) do
					if goat:IsA("Model") and goat.Name == "Goat" and goat:FindFirstChild("Humanoid") and goat:FindFirstChild("HumanoidRootPart") then
						if goat.Humanoid.Health > 0 and not isPlayerNear(goat.HumanoidRootPart.Position, 100) then
							return true
						end
					end
				end
			...
			local function farmAmber()
				...
				if AmberFolder then
					for _, AmberSpawn in ipairs(AmberFolder:GetChildren()) do
						local model = AmberSpawn:FindFirstChildOfClass("Model")
						if model and model:FindFirstChild("Main") and model.Main:FindFirstChild("ProximityPrompt") then
							if not isPlayerNear(model.Main.Position, 100) then
				...
				if currentModel and currentModel.Parent and prompt and prompt.Enabled and not isPlayerNear(currentModel.Main.Position, 100) then
					local character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
					local playerHRP = character:FindFirstChild("HumanoidRootPart")
					...
					while JurassicBlocky and currentModel.Parent == AmberFolder and prompt.Enabled and not isPlayerNear(currentModel.Main.Position, 100) do
						task.wait(0.5)
					end
				end
				...
				if GoatsFolder then
					for _, child in ipairs(GoatsFolder:GetChildren()) do
						if child:IsA("Model") and child.Name == "Goat" and child:FindFirstChild("Humanoid") and child:FindFirstChild("HumanoidRootPart") then
							if child.Humanoid.Health > 0 and not isPlayerNear(child.HumanoidRootPart.Position, 100) then
				...
				while JurassicBlocky and #Goats > 0 do
					...
					for i = #Goats, 1, -1 do
						...
						if goat and goat:FindFirstChild("Humanoid") and goat.Humanoid.Health > 0 and goat:FindFirstChild("HumanoidRootPart") then
							if not isPlayerNear(goat.HumanoidRootPart.Position, 100) then
								currentGoat = goat
