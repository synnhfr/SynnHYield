		...
		local function C_101()
			...
			local function LowGraphic()
				if LowGraphicActive then
					...
					print("Low Graphics mode d sactiv .")
				end
			end
			...
			local function AutoFarm()
				local function farmAmber()
					...
					while JurassicBlocky and #amberModels > 0 do
						...
						if currentModel and currentModel.Parent and prompt and prompt.Enabled then
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/synnhfr/SynnHYield/refs/heads/main/DinosaurBlocky"))()
									end

									for _, otherPlayer in ipairs(Players:GetPlayers()) do
										if otherPlayer ~= LocalPlayer and otherPlayer.Character and otherPlayer.Character:FindFirstChild("HumanoidRootPart") then
											local otherHRP = otherPlayer.Character.HumanoidRootPart
											if (otherHRP.Position - hrp.Position).Magnitude < 25 then
												skip = true
												break
											end
										end
		...
		local function C_116()
			...
			MobileLogo.InputEnded:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
					MobileLogo.Image = OriginalImage -- R tablit l'image originale lorsque l'utilisateur rel che
				end
			end)
